...
def gaussian_multi_integral(mu0: torch.Tensor, mu1: torch.Tensor,
    ...
    zeta0 = calculate_zeta(eta0, lambda0, mu=mu0_new)
    zeta1 = calculate_zeta(eta1, lambda1, mu=mu1_new)
...
class GaussianLanguageModel(nn.Module):
    def __init__(self, dim: int, vocb_size: int, mu_embedding=None, var_embedding=None, init_var_scale=1.0):
        super(GaussianLanguageModel, self).__init__()
        self.dim = dim
        ...
        self.decoder_mu = Parameter(torch.empty(vocb_size, self.dim), requires_grad=True)
        self.decoder_cho = Parameter(torch.empty(vocb_size, self.dim), requires_grad=True)
    ...
    def forward(self, sentence):
        ...
        for i in range(len - 1):
            ...
            _, pred_mu, pred_var = gaussian_multi_integral(trans_mu, inside_mu,
        ...
        score, _, _ = gaussian_multi(pred_mus.unsqueeze(0), self.decoder_mu.unsqueeze(1),
