...
class MixtureGaussianSequenceLabeling(nn.Module):
    ...
    def fast_forward(self, sentences: torch.Tensor) -> torch.Tensor:
        batch, max_len = sentences.size()
        swapped_sentences = sentences.transpose(0, 1)
        ...
        forward_input_mu_mat = self.input_mu_dropout(self.input_mu_embedding(swapped_sentences).
                                                     reshape(max_len, batch, self.i_comp_num, self.dim))
        forward_input_var_mat = (self.input_cho_dropout(self.input_cho_embedding(swapped_sentences)) ** 2).reshape(
            max_len, batch, self.i_comp_num, self.dim)

        # name reuse, ugly. Need to approve
        output_mu = self.output_mu_dropout(self.output_mu)
        output_var = self.output_cho_dropout(self.output_cho).reshape(self.nlabels, self.o_comp_num, self.dim) ** 2
        ...
        init_var = torch.zeros(batch, 1, 1, self.dim, requires_grad=False).to(sentences.device) + 1

        part_score, part_mu, part_var = fast_gaussian_multi_integral(trans_mu, init_mu, trans_var, init_var,
                                                                     need_zeta=True, forward=True, diag1=True)
        forward_prev_score, forward_prev_mu, forward_prev_var = gaussian_top_k_pruning(part_score.view(batch, -1),
                                                                                       part_mu.view(batch, -1,
                                                                                                    self.dim),
                                                                                       part_var.view(batch, -1,
        ...
        for i in range(max_len):
            # update forward score from pred and current inside score
            forward_score, forward_mu, forward_var = fast_gaussian_multi(
                forward_prev_mu.view(batch, -1, 1, self.dim),
                forward_input_mu_mat[i].view(batch, 1, self.i_comp_num, self.dim),
                forward_prev_var.view(batch, -1, 1, self.dim, self.dim),
                forward_input_var_mat[i].view(batch, 1, self.i_comp_num, self.dim),
                need_zeta=True, diag0=False, diag1=True)
            part_score, part_mu, part_var = gaussian_multi_integral(
                trans_mu.view(1, 1, self.t_comp_num, 2 * self.dim),
                forward_mu.view(batch, -1, 1, self.dim),
                trans_var.view(1, 1, self.t_comp_num, 2 * self.dim, 2 * self.dim),
                forward_var.view(batch, -1, 1, self.dim, self.dim), need_zeta=True, forward=True)
        ...
        # backward
        part_score, part_mu, part_var = fast_gaussian_multi_integral(trans_mu, init_mu, trans_var, init_var,
                                                                     need_zeta=True, forward=False, diag1=True)

        backward_prev_score, backward_prev_mu, backward_prev_var = gaussian_top_k_pruning(
            part_score.view(batch, -1),
            part_mu.view(batch, -1, self.dim),
            part_var.view(batch, -1, self.dim, self.dim),
            k=self.max_comp)
        ...
        backward_input_var_mat = (self.input_cho_embedding(swapped_reverted_sentences) ** 2).reshape(max_len, batch,
                                                                                                     self.i_comp_num,
                                                                                                     self.dim)
        for i in range(max_len):

            backward_score, backward_mu, backward_var = fast_gaussian_multi(
                backward_prev_mu.view(batch, -1, 1, self.dim),
                backward_input_mu_mat[i].view(batch, 1, self.i_comp_num, self.dim),
                backward_prev_var.view(batch, -1, 1, self.dim, self.dim),
                backward_input_var_mat[i].view(batch, 1, self.i_comp_num, self.dim),
                need_zeta=True, diag0=False, diag1=True)
            ...
            part_score, part_mu, part_var = gaussian_multi_integral(
                ...
                backward_mu.view(batch, -1, 1, self.dim),
                trans_var.view(1, 1, self.t_comp_num, 2 * self.dim, 2 * self.dim),
                backward_var.view(batch, -1, 1, self.dim, self.dim), need_zeta=True, forward=False)
        ...
        for i in range(0, max_len):
            ...
            next_backward_var = backward_holder_var[i + 1]
            # shape [batch, prev_comp, t_comp, dim, dim]
            part_score, part_mu, part_var = fast_gaussian_multi(
                prev_forward_mu.reshape(batch, -1, 1, self.dim),
                current_input_mu.reshape(batch, 1, self.i_comp_num, self.dim),
                prev_forward_var.reshape(batch, -1, 1, self.dim, self.dim),
                current_input_var.reshape(batch, 1, self.i_comp_num, self.dim),
                need_zeta=True, diag0=False, diag1=True)

            part_score = part_score + prev_forward_score.reshape(batch, -1, 1)
            expected_count_score, expected_count_mu, expected_count_var = gaussian_multi(
                part_mu.reshape(batch, -1, 1, self.dim),
                next_backward_mu.reshape(batch, 1, -1, self.dim),
                part_var.reshape(batch, -1, 1, self.dim, self.dim),
                next_backward_var.reshape(batch, 1, -1, self.dim, self.dim),
                need_zeta=True)

            real_expected_score = part_score.view(batch, -1, 1) + expected_count_score + next_backward_score.view(batch,
                                                                                                                  1, -1)

            pruned_score, pruned_mu, pruned_var = gaussian_top_k_pruning(
                real_expected_score.reshape(batch, -1),
                expected_count_mu.reshape(batch, -1, self.dim),
        ...
        # DEBUG debug the length here.
        score, _, _ = fast_gaussian_multi(
            expected_count_mus.view(batch, max_len, 1, -1, 1, self.dim),
            output_mu.view(1, 1, self.nlabels, 1, self.o_comp_num, self.dim),
            expected_count_vars.view(batch, max_len, 1, -1, 1, self.dim, self.dim),
            output_var.view(1, 1, self.nlabels, 1, self.o_comp_num, self.dim),
            need_zeta=True, diag0=False, diag1=True)
    ...
    def get_loss(self, sentences: torch.Tensor, labels: torch.Tensor,
        ...
        if wishart:
            ...
            reg = torch.sum(InvWishart.logpdf(trans_cho, 2 * self.dim, torch.eye(2 * self.dim)))
        else:
            reg = 0.0
...
# The fash method may need despatched, but now it used as backup.
# cho init 0 means random, else means init scale
class FastMixtureGaussianSequenceLabeling(nn.Module):
    def __init__(self, dim: int, ntokens: int, nlabels: int,
                 in_cho_init=0, t_cho_init=0, out_cho_init=0,
                 t_cho_method='random', mu_embedding=None, var_embedding=None,
                 i_comp_num=1, t_comp_num=1, o_comp_num=1, max_comp=1,
                 in_mu_drop=0.0, in_cho_drop=0.0, out_mu_drop=0.0,
                 out_cho_drop=0.0, t_mu_drop=0.0, t_cho_drop=0.0):
        super(FastMixtureGaussianSequenceLabeling, self).__init__()
    ...
    def get_inverse_var(self, sentences):
        inverse_var = []
        for idx in range(sentences.size(0)):
            if sentences[idx].tobytes() in self.stored_var_inverse:
                inverse_var.append(self.stored_var_inverse[sentences[idx].tobytes()])
            else:
                pass
        return inverse_var
